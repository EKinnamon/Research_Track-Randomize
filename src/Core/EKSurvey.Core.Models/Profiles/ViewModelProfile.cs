    ...
    public class ViewModelProfile : Profile
    {
        public ViewModelProfile()
        {
            ...
            CreateMap<ICollection<UserSurvey>, IndexViewModel>()
                .ForMember(dest => dest.AvailableSurveys, opt => opt.MapFrom(src => new HashSet<UserSurvey>(src.Where(i => !i.Completed.HasValue).OrderBy(i => i.Modified.GetValueOrDefault(i.Created)))))
                .ForMember(dest => dest.CompletedSurveys, opt => opt.MapFrom(src => new HashSet<UserSurvey>(src.Where(i => i.Completed.HasValue))))
                .ForMember(dest => dest.NextSurvey, opt => opt.MapFrom(src => src.Where(i => !i.Completed.HasValue).OrderBy(i => i.Started.GetValueOrDefault(DateTime.UtcNow)).ThenBy(i => i.Modified.GetValueOrDefault(i.Created)).FirstOrDefault()));
